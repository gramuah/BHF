...
public:
    ...
    // detection
    void DetectList();
    //void DetectPyramid(const cv::Mat img, std::vector<std::vector<cv::Mat> >& hough_maps, std::vector<std::vector<std::map<long unsigned int, float> > >& hough_pose_maps, std::vector<std::vector<std::vector<std::vector<Vote> > > >& vBackprojections, cv::Rect ROI = cv::Rect(-1,-1,-1,-1), int index = -1);
...
private:
    ...
    //void DetectImage(const cv::Mat img, std::vector<cv::Mat>& imgDetect, std::vector<std::map<long unsigned int, float> >& imgDetectPose, std::vector<std::vector<std::vector<Vote> > >& backprojections, cv::Rect ROI = cv::Rect(-1,-1,-1,-1), int index = -1);
	...
	//void DetectBoundingBoxes(cv::Mat img, vector<vector<cv::Mat> >& hough_maps, std::vector<std::vector<std::map<long unsigned int, float> > >& hough_pose_maps, std::vector<std::vector<std::vector<std::vector<Vote> > > >& vBackprojections, cv::Size avg_bbox_size, int img_id);
