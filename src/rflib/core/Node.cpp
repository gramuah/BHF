...
template<typename Sample, typename Label, typename SplitFunction, typename LeafNodeStatistics, typename AppContext>
void Node<Sample, Label, SplitFunction, LeafNodeStatistics, AppContext>::Save(std::ofstream& out, Eigen::MatrixXd& latent_variables)
{
	out << this->m_node_id << " " << this->m_depth << " " << this->m_is_leaf << std::endl;
	if (m_is_leaf)
		this->m_leafstats->Save(out, latent_variables);
	else
