...
template<typename Sample, typename Label, typename SplitFunction, typename SplitEvaluator, typename LeafNodeStatistics, typename AppContext>
void
RandomTree<Sample, Label, SplitFunction, SplitEvaluator, LeafNodeStatistics, AppContext>::Save(std::ofstream& out, Eigen::MatrixXd& latent_variables)
{
	...
	for (int n = 0; n < m_nodes.size(); n++)
		m_nodes[n]->Save(out, latent_variables);
}
